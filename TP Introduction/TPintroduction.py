...
plt.plot(histocum)
plt.savefig('lena_cumhist_noise_stddev250.jpg')
plt.clf()
...
histocum=histo.cumsum()
plt.plot(histocum)
plt.savefig('histo_sombre.jpg')
plt.clf()
imequal=histocum[np.uint8(im)]
#viewimage(imequal)

(histo,bins)=np.histogram(imequal.reshape((-1,)),np.arange(0,1, 1/256)) #le reshape est inutile pour np.histogram, mais on le laisse pour la compatibilite avec plt.hist
histo=histo/histo.sum()
histocum=histo.cumsum()
plt.plot(histocum)
plt.savefig('histo_sombre_egal.jpg')
plt.clf()
...
viewimage(abs(np.float32(unew)-np.float32(v)), normalise=True)
...
viewimage(seuil(noise(im,40),56)) #exemple de dithering

## Diff de niveaux de gris voisins
im=skio.imread('images/lena.tif')
im = gradx(im)
viewimage(im)
plt.plot(np.histogram(im,255)[0])
plt.savefig('histo_nivgris.jpg')
plt.clf()

#%% log d'un histogramme
plt.plot(np.log(np.histogram(gradx(im),255)[0]))
plt.savefig('histo_nivgris_log.jpg')
plt.clf
#%%
